...
export class AuthTeacherService {
  constructor(
    private readonly usersService: TeacherService,
    private readonly jwtService: JwtService,
  ) {}

  async generateTokens(user: Teacher) {
    const payload = {
      id: user.id,
      email: user.email,
      role: user.role,
  ...
  async signIn(signInDto: SignInDto, res: Response) {
    const user = await this.usersService.findTeacherByEmail(signInDto.email);
    if (!user) {
  ...
  async signOut(refreshToken: string, res: Response) {
    ...
    const response = {
      message: "Teacher logged out seccessfully",
    };
    return response;
  }
