...
export class AuthStudentService {
  constructor(
    private readonly usersService: StudentsService,
    private readonly jwtService: JwtService,
  ) {}

  async generateTokens(user: Student) {
    const payload = {
      id: user.id,
      email: user.email,
      role:"student",
  ...
  async signIn(signInDto: SignInDto, res: Response) {
    const user = await this.usersService.findStudentByEmail(signInDto.email);
    if (!user) {
  ...
  async signOut(refreshToken: string, res: Response) {
    ...
    const response = {
      message: "Student logged out seccessfully",
    };
    return response;
  }
